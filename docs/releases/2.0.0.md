...
### Dual-mode operation
- Mission Control now supports both Hermes gateway mode and local workstation mode cleanly.
...
- Hermes coverage was expanded across channels, chat, sessions, cron, devices, approvals, logs, usage, and config editing.
...
- Hermes doctor warnings, doctor fix, and state-integrity handling are now surfaced directly in the UI.
...
- Agent/workspace creation and deletion flows were hardened end to end, including Hermes config normalization and cleanup.
...
- Local mode and Hermes gateway mode were hardened into first-class operating modes.
...
- Mission Control can register itself as the dashboard for an Hermes runtime without the earlier device-auth and dashboard URL failure cases.
...
- Session transcript handling improved across local, Hermes, aggregated gateway, and disk-backed Hermes transcript sources.
...
- Task dispatch now polls assigned tasks and runs agents through Hermes with session linkage.
...
- Security scan severity scoring, infrastructure checks, Hermes hardening checks, autofix behavior, and agent-side scan APIs improved.
...
- Branding assets and logos for Mission Control, Hermes, Claude, Codex, and Hermes were refreshed across the interface.
...
- Hermes doctor drift handling, doctor fix, config normalization, and workspace lifecycle cleanup were tightened to better support real self-hosted installs.

### E2E, CI, and release readiness
- The E2E harness now uses isolated Hermes runtime state, isolated skill roots, deterministic scheduler behavior, and safer env handling.
...
- Hermes backup creation and restore-oriented backup directory handling
...
- Hermes update-now trigger from the UI
- Approvals overlay, channels parity, chat parity, usage parity, device parity, and logs parity with the newer Hermes surface
...
- If you run Hermes, use the in-app doctor and security panels after upgrade to catch any stale config drift.
