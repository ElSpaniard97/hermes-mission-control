...
- Gateway startup and runtime checks now better tolerate read-only configs, missing `HERMES_HOME`, and container health-probe requirements.
...
- Docker/runtime regressions around Compose, assets, probes, `HERMES_HOME`, and read-only configs
...
- Hermes spawn compatibility when agents use configured default models
