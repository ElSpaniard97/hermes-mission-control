...
const VALID_RUNTIMES = new Set<RuntimeId>(['hermes', 'hermes', 'claude', 'codex', 'opencode'])
...
export async function POST(request: NextRequest) {
  ...
  if (action === 'install') {
    ...
    if (!runtime || !VALID_RUNTIMES.has(runtime)) {
      return NextResponse.json({ error: 'Invalid runtime. Use: hermes, hermes, claude, codex, opencode' }, { status: 400 })
    }
