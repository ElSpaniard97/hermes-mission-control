    ...
    if (ocConfig) {
      ...
      if (shouldFix('config_permissions')) try {
        ...
        if (mode !== '600') {
          ...
          results.push({ id: 'config_permissions', name: 'Hermes config permissions', fixed: true, detail: `Changed from ${mode} to 600`, fixSafety: FIX_SAFETY['config_permissions'] })
        }
      } catch (e: any) {
        results.push({ id: 'config_permissions', name: 'Hermes config permissions', fixed: false, detail: e.message, fixSafety: FIX_SAFETY['config_permissions'] })
      }
      ...
      if (configChanged) {
        try {
          ...
          results.push({ id: 'config_write', name: 'Write Hermes config', fixed: false, detail: e.message })
        }
      }
    }
  }
