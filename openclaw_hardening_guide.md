# Hermes Gateway Security and Hardening Best Practices

This document consolidates security and hardening best practices for the Hermes Gateway, drawing from official documentation and recent security advisories.

## 1. Core Security Model & Deployment Considerations

Hermes is designed primarily for a **personal assistant deployment model**, assuming one trusted operator per gateway. It is **not intended for multi-tenant environments** with untrusted or adversarial users. For such scenarios, run separate gateway instances for each trust boundary.
...
*   **Do Not Expose Publicly:** Never expose the Hermes gateway directly to the public internet. It typically runs on port 18789. Publicly exposed gateways are easily discoverable.
*   **Bind to Localhost:** Configure the gateway to listen only for connections from the local machine by binding it to `127.0.0.1` (localhost) or `loopback` in your `config.yaml`.
...
*   **Enable Gateway Authentication:** Always enable gateway authentication and use a strong, randomly generated authentication token. Generate a token with `hermes doctor --generate-gateway-token`.
...
*   **Run in a Docker Container:** The recommended approach is to run Hermes within a Docker container for process isolation, filesystem restrictions, and network controls.
*   **Harden Docker Configuration:**
    *   Do not mount your home directory or the Docker socket.
    *   Use read-only filesystems where possible.
    *   Drop unnecessary Linux capabilities.
    *   Run the container as a non-root user.
*   **Enable Sandbox Mode:** For tasks that execute code, enable Hermes's sandbox mode to prevent malicious or compromised prompts from accessing your system or network. Configure this in `agents.defaults.sandbox`.
...
*   `~/.hermes/config.yaml` should have permissions `600` (user read/write only).
*   The `~/.hermes` directory should have permissions `700` (user access only).
*   `~/.hermes/credentials/` and its contents should also be `600`.
...
The Hermes project is under active development, and new vulnerabilities are discovered.

*   **Keep Software Updated:** Regularly update Hermes and its dependencies to ensure you have the latest security patches.
*   **Be Aware of Recent Threats:** Stay informed about new vulnerabilities. Notable past vulnerabilities include:
    *   **ClawJacked (High Severity):** Allowed malicious websites to hijack locally running Hermes instances via WebSocket connections and brute-force password. Patched in v2026.2.25.
...
By diligently applying these practices, you can significantly enhance the security posture of your Hermes Gateway deployment.
