...
# Run offline Hermes harness (no Hermes install required)
...
## Hermes Offline Harness

The harness runs Mission Control against fixture data and mock binaries/gateway:
- fixtures: `tests/fixtures/hermes/`
- mock CLI: `scripts/e2e-hermes/bin/{hermes,hermesbot}`
- mock gateway: `scripts/e2e-hermes/mock-gateway.mjs`

Profiles:
- `pnpm test:e2e:hermes:local` - local mode (gateway not running)
- `pnpm test:e2e:hermes:gateway` - gateway mode (mock gateway running)
